...
class AdminVerifyService
{
    ...
    {
        $admin_verify  = Config::get("verify");;
        return $admin_verify;
    }
    ...
    protected function configure(string $config = null): void
    {
        ...
        if ($admin_verify) {
            ...
            if ($admin_verify['type'] == 1) {
                $this->codeSet = '0123456789';
            } elseif ($admin_verify['type'] == 2) {
    ...
    protected function generate(): array
    {
        ...
        if ($this->math) {
            ...
            AdminVerifyCache::set($key, $and, $this->expire);
        } else {
            if ($this->useZh) {
            ...
            AdminVerifyCache::set($key, $val, $this->expire);
        }
    ...
    {
        $verify = AdminVerifyCache::get($verify_id);
    ...
    public function create(string $config = null, bool $api = false)
    {
        ...
        // 验证码使用随机字体
        $ttfPath = root_path() . 'public/static/verify/assets/' . ($this->useZh ? 'zhttfs' : 'ttfs') . '/';
